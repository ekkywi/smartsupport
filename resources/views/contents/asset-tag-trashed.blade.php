...
@section("title")
    SmartSupport &mdash; Tag Aset Terhapus (Trash)
@endsection
...
@section("content")
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">Tag Aset Terhapus (Trash)</h1>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb breadcrumb-style2 mb-0">
                        ...
                        <li aria-current="page" class="breadcrumb-item active">Tag Aset Terhapus</li>
                    </ol>
                </nav>
            </div>
        </div>
        ...
        <div class="mb-3 d-flex flex-wrap gap-2">
            <a class="btn btn-secondary mt-3" href="{{ route("asset.tags.index") }}">
                <i class="ti ti-arrow-left"></i> Kembali ke Data Tag Aset
            </a>
        </div>
                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table class="table table-bordered text-nowrap" id="responsiveDataTable" style="width:100%">
                            ...
                            <tbody>
                                @forelse($trashedAssetTags as $assetTag)
                                    <tr>
                                        <td>{{ $assetTag->name }}</td>
                                        <td>{{ $assetTag->asset_tag }}</td>
                                        <td>{{ $assetTag->description }}</td>
                                        <td>{{ $assetTag->deleted_at ? $assetTag->deleted_at->format("d-m-Y H:i") : "-" }}</td>
                                        <td>
                                            <form action="{{ route("asset.tags.restore", $assetTag->id) }}" method="POST" style="display:inline;">
                                                @csrf
                                                <button class="btn btn-success btn-sm" type="submit">
                                                    <i class="ti ti-history"></i> Restore
                                                </button>
                                            </form>
                                            <form action="{{ route("asset.tags.force.delete", $assetTag->id) }}" id="delete-form-{{ $assetTag->id }}" method="POST" style="display: none;">
                                                @csrf
                                                @method("DELETE")
                                            </form>
                                            <button class="btn btn-danger btn-sm delete-btn" data-assettag-id="{{ $assetTag->id }}">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.delete-btn', function(e) {
                e.preventDefault();
                var assetTagId = $(this).data('assettag-id');
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        document.getElementById("delete-form-" + assetTagId).submit();
                    }
                });
            });
        });
