...
@section("title")
    SmartSupport &mdash; Vendor Terhapus
@endsection
...
@section("content")
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">Vendor Terhapus (Trash)</h1>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb breadcrumb-style2 mb-0">
                        ...
                        <li class="breadcrumb-item active">Vendor Terhapus</li>
                    </ol>
                </nav>
            </div>
        </div>
        ...
        <div class="mb-3 d-flex flex-wrap gap-2">
            <a class="btn btn-secondary mt-3" href="{{ route("vendors.index") }}">
                <i class="ti ti-arrow-left"></i> Kembali ke Data Vendor
            </a>
        </div>
                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table class="table table-bordered text-nowrap" id="responsiveDataTable" style="width:100%">
                            ...
                            <tbody>
                                @forelse($trashedVendors as $vendor)
                                    <tr>
                                        ...
                                        <td>{{ $vendor->notes }}</td>
                                        <td>
                                            <form action="{{ route("vendors.restore", $vendor->id) }}" method="POST" style="display:inline;">
                                                @csrf
                                                <button class="btn btn-success btn-sm" type="submit">
                                                    <i class="ti ti-history"></i> Restore
                                                </button>
                                            </form>
                                            <form action="{{ route("vendors.force.delete", $vendor->id) }}" id="delete-form-{{ $vendor->id }}" method="POST" style="display: none;">
                                                @csrf
                                                @method("DELETE")
                                            </form>
                                            <button class="btn btn-danger btn-sm delete-btn" data-vendor-id="{{ $vendor->id }}">
