...
@section("content")
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">Tag Aset</h1>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb breadcrumb-style2 mb-0">
                        <li class="breadcrumb-item">Master Data Aset</li>
                        <li class="breadcrumb-item">Penomoran Aset</li>
                        <li aria-current="page" class="breadcrumb-item active">Tag Aset</li>
                    </ol>
                </nav>
            </div>
        </div>
        ...
        <div class="mb-3 d-flex flex-wrap gap-2">
            <a class="btn btn-success" href="{{ route("asset.tags.create") }}">
                <i class="ti ti-plus"></i> Tambah Tag Aset
            </a>
            <a class="btn btn-primary" href="{{ route("asset.tags.trashed") }}">
                            ...
                            <tbody>
                                @forelse ($assetTags as $assetTag)
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-sm btn-primary" href="{{ route("asset.tags.edit", $assetTag->id) }}"><i class="ti ti-pencil"></i> Edit</a>
                                            <button class="btn btn-sm btn-danger delete-btn" data-asset-tag-id="{{ $assetTag->id }}" type="button"><i class="ti ti-trash me-1"></i>
                                                Hapus
                                            </button>
                                            <form action="{{ route("asset.tags.destroy", $assetTag->id) }}" id="delete-form-{{ $assetTag->id }}" method="POST" style="display: none;">
                                                @csrf
                                                @method("DELETE")
                                            </form>
                                        </td>
...
@section("scripts")
    ...
    <script>
        $(document).on('click', '.delete-btn', function(e) {
            e.preventDefault();
            var assetTagId = $(this).data('asset-tag-id');
            Swal.fire({
