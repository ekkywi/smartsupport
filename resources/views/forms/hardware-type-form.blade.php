...
@section("title")
    SmartSupport &mdash; {{ isset($hardwareType) ? "Edit" : "Tambah" }} Jenis Hardware
@endsection
            ...
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb breadcrumb-style2 mb-0">
                        ...
                        <li aria-current="page" class="breadcrumb-item active">{{ isset($hardwareType) ? "Edit" : "Tambah" }} Jenis Hardware</li>
                    </ol>
                </nav>
            </div>
        </div>
                        ...
                        <div class="row gy-4">
                            <div class="card-header justify-content-between">
                                <div class="card-title">
                                    Form {{ isset($hardwareType) ? "Edit" : "Tambah" }} Jenis Hardware
                                </div>
                            </div>
                            <form action="{{ isset($hardwareType) ? route("hardware.types.update", $hardwareType->id) : route("hardware.types.store") }}" autocomplete="off" class="row gy-4" method="POST">
                                @csrf
                                @if (isset($hardwareType))
                                    @method("PUT")
                                @endif
                                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12">
                                    <label class="form-label text-default fw-semibold" for="name">Nama Jenis Hardware</label>
                                ...
                                <div class="col-12 d-flex justify-content-end gap-2 mt-4">
                                    ...
                                    <button class="btn btn-primary" type="submit"><i class="ti ti-check"></i> {{ isset($hardwareType) ? "Update" : "Simpan" }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
@section("scripts")
    ...
    <script>
        function clearFormInputs() {
            ...
            document.getElementById('hardware_type_tag').value = '';
        }

        function goToIndex() {
            window.location.href = "{{ route("hardware.types.index") }}";
        }
    </script>
@endsection
